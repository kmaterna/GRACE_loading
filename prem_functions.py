...
def compute_prem_load(station_array, load_array, disk_radius, greensfunc, max_distance):
	...
	for i in range(len(station_array.lon)):  # for each station... 
		...
		for j in range(np.size(CircleLoads.lon)):
			...
			dist_from_center=haversine.distance([CircleLoads.lat[j], CircleLoads.lon[j]],[station_array.lat[i], station_array.lon[i]]);  # reported in km
			azimuth=haversine.calculate_initial_compass_bearing((station_array.lat[i], station_array.lon[i]), (CircleLoads.lat[j], CircleLoads.lon[j]));
			myindex = find_closest_greens(greensfunc.km, dist_from_center);   # find the nearest answer in the Green's functions
			vdisp= greensfunc.vload[myindex];  # in m
			hdisp= greensfunc.hload[myindex];  # in m
			[xdisp, ydisp] = az_decompose(hdisp,(station_array.lat[i], station_array.lon[i]), (CircleLoads.lat[j], CircleLoads.lon[j])); # Decompose radial into x and y components
			
			disk_sum_v=disk_sum_v+vdisp*CircleLoads.pressure[j]*1000/9.81; # THIS SHOULD BE DIVIDED BY 9.81! the PREM code already has it. Result in mm
	...
	return total_disp_x, total_disp_y, total_disp_v;  # This should exactly match the length of the input station_array fields. 
...
def get_disks(load_array,disk_radius,threshold_distance,sta_lon,sta_lat):
	...
	for i in range(len(load_array.lon)): # for each rectangular load cell
		if abs(load_array.lat[i])>85:  # ignoring the arctic and antarctic
			continue;
		dist_from_center = haversine.distance([load_array.lat[i], load_array.lon[i]],[sta_lat, sta_lon]);  # reported in km
		if dist_from_center < threshold_distance:  # if the load cell is less than a certain distance from the station, discretize it. 
			...
			n_tiles_y = int(np.floor(load_bkm*2 / square_side_length));  # the number of tiles that fit in x and y dimensions of the rectangle	
			x_array=[]; y_array=[];
			x_nodes = np.linspace(center[0]-load_array.east_width[i], center[0]+load_array.east_width[i], n_tiles_x+1, endpoint=True);
			y_nodes = np.linspace(center[1]-load_array.north_width[i], center[1]+load_array.north_width[i], n_tiles_y+1, endpoint=True);
	...
	CircleLoads = helper_functions.Load_Array(lon=circle_lons, lat=circle_lats, east_width=circle_radius, north_width=[], pressure=circle_pressure);
	return CircleLoads;
