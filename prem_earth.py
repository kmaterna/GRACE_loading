...
def prem_earth_grace_timeseries(params):
    ...
    for i in range(len(station_obj.name)):  # once for each station
        single_station = helper_functions.Station_Array(name=[station_obj.name[i]], lon=[station_obj.lon[i]],
                                                        lat=[station_obj.lat[i]],
                                                        start_time=[station_obj.start_time[i]],
                                                        end_time=[station_obj.end_time[i]]);
    ...
    def mass_loading_TS_python_implementation(GRACE_array, single_station):
        ...
        for n in range(len(GRACE_array.grace_central_times)):  # for each snapshot:
            if GRACE_array.grace_central_times[n] < single_station.start_time[0] or GRACE_array.grace_central_times[n] > single_station.end_time[0]:
                ...
                # more basic structure of the loads
                GRACE_single_loads = helper_functions.package_loads_from_GRACE(GRACE_array, n, params.scaling);
                x, y, z = prem_functions.compute_prem_load(single_station, GRACE_single_loads, disk_radius, GF,
                                                           params.max_distance);
                disp_x.append(x[0]);
                disp_y.append(y[0]);
...
def outputs(dates, disp_x, disp_y, disp_v, params, single_station):
    ...
    output_dir = params.output_dir + "/" + params.datasource + "_" + params.gracetype + "_" + str(
        params.scaling) + "/";  # Example: "MIBB/JPL_TELLUS_0/"
