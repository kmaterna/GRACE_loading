...
def elastic_earth_computation(network, dataset, use_scale):
	[netfile, scalefile, station_file, output_dir, file_prefix, mu, lame, threshhold_distance] = configure(network, dataset, use_scale);
...
def configure(network, dataset, use_scale):
	...
	helper_functions.delete_files_matching(output_dir+file_prefix+'*_halfspace_model_ts.txt');  # get started with a clean directory. 
	...
	# THE COMPUTATION LOOP
 	for i in range(len(my_station_array.station_names)):
		...
		compute_load_ts(my_station_array.start_time[i], my_station_array.end_time[i], my_station_array.station_lon[i], my_station_array.station_lat[i], 
			"temp_my_station.txt", "temp_GRACE_load.txt", output_file, mu, lame, threshhold_distance, use_scale, grace_data_structure);
	return;


def compute_load_ts(start_date, end_date, station_lon, station_lat, stationfile, loadfile, output_file, lame, mu, threshhold_distance, use_scale, grace_data_structure):
	"""
	...
	for n in range(len(grace_data_structure.grace_start_times)):  # for each snapshot: 
		if grace_data_structure.grace_start_times[n]<start_date or grace_data_structure.grace_start_times[n]>end_date:
			...
			helper_functions.write_station_file(stationfile, station_lon, station_lat);  # write a file with one station's coordinate. 
			helper_functions.write_time_stamp(output_file,grace_data_structure.layerstrings[n]);  # write the time stamp into the output file. 

			# Do the computation: Mode 1 = time series (append to the output file). Mode 2 = image (just write the output file). 
			call(['./a',stationfile,loadfile,output_file,str(lame),str(mu),str(threshhold_distance),str(1)],shell=False);
	return;
